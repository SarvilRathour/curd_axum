use axum::{extract::{FromRef,FromRequestParts},http::{request::Parts,StatusCode,HeaderMap}};
use uuid::Uuid;
...
where 
  ...
  S:Send+Sync
  {
    ...
    async fn from_request_parts(parts:&mut Parts,state:&s)->Result<Self,Self::Rejection>{
      ...
      let user=app_state.user_repository.find_by_id(user_id).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?
    ...
    S:Send+Sync,
    {
      ...
      async fn from_request_parts(parts:&mut Parts,state:&S)->Result<Self,Self::Rejection>{
        ...
        let user_id=match Uuid::parse_str(&claims.sub){
          Ok(id)=>id,
          Err(_)=>return Ok(OptinalAuth(None)),
        };
        let user=app_state.user_repository.find_by_id(user_id).await.map_err(|_| Status::INTERNAL_SERVER_ERROR)?;
